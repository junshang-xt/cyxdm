// 页面模板
const pageTemplate = (pageNumber) => `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>页面 ${pageNumber} - 创意小代码</title>
    ...
    <style>
        ...
        .page-title {
            font-size: 2.5rem;
            margin-bottom: 30px;
            background: linear-gradient(135deg, #${getColor(pageNumber)}.html, #${getSecondColor(pageNumber)});
...
<body>
    ...
    <div class="container">
        <div class="content-container">
            <h1 class="page-title">页面 ${pageNumber}</h1>
            
            <div class="feature-box">
                <h2>${getPageTitle(pageNumber)}</h2>
                <p class="page-content">${getContent(pageNumber)}</p>
            </div>
            
            <div class="feature-box">
                <h3>核心功能</h3>
                <ul style="text-align: left; max-width: 600px; margin: 0 auto;">
                    ${getFeatures(pageNumber).map(feature => `<li style="margin: 10px 0; color: rgba(255, 255, 255, 0.8);">${feature}</li>`).join('\n                    ')}
                </ul>
            </div>
...
// 根据页面编号生成不同的颜色
function getColor(pageNumber) {
    const colors = [
        '64ffda', '6495ff', '9664ff', 'ff64aa', 'ff9564', '64ff95',
        'da64ff', 'ffda64', '64daff', 'ff6464', '95ff64', '6464ff'
    ];
    return colors[(pageNumber - 1) % colors.length];
}

function getSecondColor(pageNumber) {
    const colors = [
        '40e0d0', '4169e1', '7b68ee', 'ff69b4', 'ffa500', '32cd32',
        '9370db', 'ffd700', '4682b4', 'ff6347', '7cfc00', '483d8b'
    ];
    return colors[(pageNumber - 1) % colors.length];
}
...
function getPageTitle(pageNumber) {
    ...
    return titles[(pageNumber - 1) % titles.length];
}
...
function getContent(pageNumber) {
    ...
    return contents[(pageNumber - 1) % contents.length];
}

// 根据页面编号生成不同的功能列表
function getFeatures(pageNumber) {
    const featureSets = [
    ...
    return featureSets[(pageNumber - 1) % featureSets.length];
}

// 输出生成说明（在Node.js环境中会执行，在浏览器中不会）
if (typeof window === 'undefined') {
    console.log('此脚本用于生成页面模板，实际生成需要在Node.js环境中执行');
}